...
public class PRNGenerator {
    ...
    public HashMap<String, String> extract_invoice_data(String file_name, int page_count) {
        ...
        if (orientation != null) {
            System.out.println(TAG + ": extract_invoice_data(): document orientation: " + orientation);
            ...
            String msg = TAG + ": extract_invoice_data(): Failed to get orientation for file: " + file_name;
    ...
    HashMap<String, String> extract_single_page_data(String template, String file_path_name, int page_number) {
        System.out.println(TAG + "extract_single_page_data(): running..template: " + template);
        ...
        String status_msg = TAG + ": extract_single_page_data(): Config.INVOICE_DATA_BOUNDERIES:";
        ...
        status_msg = TAG + ": extract_single_page_data(): Template bounderies:";
        System.out.println(status_msg);
        ...
        for (Object key : invoice_type_object.keySet()) {
            ...
            if (pageText != null) {
                ...
                if (!extracted_value.contains("Error #")) {
                    ...
                    if (required_data_field.equalsIgnoreCase("Grand Total")) {
                        String currency = "ZWL";
                        if (extracted_value.contains("ZWL")) {
                            currency = "ZWL";
                        }
                        if (extracted_value.contains("US")) {
                            currency = "US";
                        }
                    ...
                    System.out.println(msg);
                    //Put default values
                    if (required_data_field.equalsIgnoreCase("Invoice Number")) {
                        String default_value = "100001";
                        extracted_data_values.put(required_data_field, default_value);
                    } else if (required_data_field.equalsIgnoreCase("Customer VAT Number")) {
    ...
    HashMap<String, String> extract_multi_page_data(String template, String file_path_name, int page_count) {
        System.out.println(TAG + ": extract_multi_page_data(): running... : template: " + template + ", page_count: " + page_count);
        HashMap<String, String> extracted_data = new HashMap();
        //Check which templates to use
        if (page_count == 2) {
            //Document has only first & last page
            for (int i = 0; i < page_count; i++) {
                System.out.println(TAG + ": extract_multi_page_data(): 2 paged, current loop i = " + i);
                if (i == 0) {
            ...
            System.out.println(TAG + ": extract_multi_page_data(): no matching case for page_count: " + page_count);
        }
    ...
    public HashMap<String, HashMap<Integer, ArrayList<String>>> generatePRNData(HashMap<String, String> invoice_data_values) {
        ...
        for (Object key : invoice_data_values.keySet()) {
            ...
            if (!field_name.equalsIgnoreCase("Line items")) {
                ...
                if (field_name.equalsIgnoreCase("Invoice Number")) {
                    if (!field_value.equalsIgnoreCase("100001")) {
                        //Parse invoice number 
                        String invoice_number = parse_invoice_number(field_value);
                        field_value = invoice_number;
                    }
                }

                if (field_name.equalsIgnoreCase("Customer VAT Number")) {
                    //Parse vat number 
                    //Sample: - 10052137
                    String vat_number = field_value.replace("-", "").trim();
                    field_value = vat_number;
                }

                if (field_name.equalsIgnoreCase("Grand Total")) {
                    //Parse grand total from sample: US$ 16,150.00
                    String grand_total = "0.00";
                    if (field_value.contains("US$")) {
                        grand_total = field_value.replace("US$", "").replace(",", "").trim();
                    } else if (field_value.contains("ZWL")) {
                        grand_total = field_value.replace("ZWL", "").replace(",", "").trim();
                    }
                    field_value = grand_total;
                }
                ...
                if (invoice_type.contains("landscape")) {
                    ...
                    if (invoice_type.equalsIgnoreCase("landscape invoice")) {
                        line_items_map = ambus_process_single_page_landscape_invoice_line_items(field_value);
                    } else if (invoice_type.equalsIgnoreCase("landscape multipage")) {
                        line_items_map = ambus_process_multipage_page_landscape_invoice_line_items(field_value);
                    }
    ...
    String parse_invoice_number(String field_value) {
        /*
         Sample input string:
         - 10052137
         G-INK004235
        ...
        String[] lines = field_value.split("\n");
        //Get the invoice number at index 2
        return lines[2];
    ...
    /*
     Sample:
     105-CHROME LAMP - STOP 19 LED RED 10-30V STEEL 2.00 3,400.001700.00
     106-CHROME LAMP - INDICATOR 19 LED ORA 10-30V STEEL 5.00 8,500.001700.00
     107-CHROME LAMP - REVERSE 19 LED CLEAR 10-30V STEEL 2.00 4,250.002125.00
     */
    HashMap<Integer, ArrayList<String>> ambus_process_single_page_landscape_invoice_line_items(String line_items_str) {
        ...
        for (int entry_index = 0; entry_index < line_item_entries.length; entry_index++) {
            ...
            System.out.println(entry_index + ": line_item_entry: " + line_item_entry);
            //Explode line_item_entry into words along space characters
            String[] line_item_entry_words = line_item_entry.split(" ");
            //Remove empty elements
            ArrayList<String> non_empty_values = new ArrayList();
            for (int e = 0; e < line_item_entry_words.length; e++) {
                String element = line_item_entry_words[e];
                //Trim this element
                element = element.trim();
                //Check if it still contains characters
                if (element.length() > 0) {
                    //Add to non emepty arraylist
                    non_empty_values.add(element);
                }
            }

            if (non_empty_values.size() > 0) {
                //Print out the contents
                /*
                 for(int c=0;c<non_empty_values.size();c++){
                 System.out.println(c + ": " + non_empty_values.get(c));
                 }*/
                ...
                for (int c = 0; c < non_empty_values.size(); c++) {
                    if (c < (non_empty_values.size() - 2)) {
                        if (non_empty_values.get(c).equalsIgnoreCase("-")) {
                            line_item_desc = line_item_desc + non_empty_values.get(c);
                        } else {
                            line_item_desc = line_item_desc + " " + non_empty_values.get(c);
                        }
                    } else if (c == (non_empty_values.size() - 2)) {
                        line_item_qty = non_empty_values.get(c);
                    } else if (c == (non_empty_values.size() - 1)) {
                        //Sample:  4,250.002125.00
                        String item_totals_str = non_empty_values.get(c).trim();
                        //Locate the first dot seperator
                        int index_of_first_dot = item_totals_str.indexOf(".");
                        if (index_of_first_dot != -1) {
                            //Extract the unit price
                            unit_price_excl_discount = item_totals_str.substring(0, index_of_first_dot + 3);
                            //Extract subtotal
                            total_all_incl = item_totals_str.substring(index_of_first_dot + 3);
                        } else {
                            //Dot seperator not found
                            System.out.println("Invalid item totals string..");
                        }
                    }
                }
    ...
    HashMap<Integer, ArrayList<String>> ambus_process_multipage_page_landscape_invoice_line_items(String line_items_str) {
        System.out.println("ambus_process_multipage_page_landscape_invoice_line_items() running... ");
        HashMap<Integer, ArrayList<String>> line_items = new HashMap();
        //Explode the string along \n characters
        String[] line_item_entries = line_items_str.trim().split("\n");
            ...
            for (int i = 0; i < field_values_map.size(); i++) {
                ...
                if (!field_name.equalsIgnoreCase("Line items")) {
                    ...
                    for (int entry_index = 0; entry_index < field_values_map.size(); entry_index++) {
                        ...
                        System.out.println(TAG + ", generatePRN(): input entry_values: " + entry_values);
                        System.out.println(entry_values);
                        ...
                        if (invoice_type.contains("landscape")) {
                            ...
                            System.out.println(TAG + ", generatePRN(): input item_qty: " + item_qty);
