...
import invoicetypeutils.Printer;
...
public class BackgroundWorker {
    ...
    public static int total_signed_invoices_printed = 0;
    public static HashMap<String, ArrayList<String>> outbox = new HashMap();
    public static HashMap<String, ArrayList<String>> print_queuee = new HashMap();
    ...
    public BackgroundWorker() {
        ...
        //Start the CheckAndPrintInvoicesThread
        CheckAndPrintInvoicesThread print_invoices_thread = new CheckAndPrintInvoicesThread();
        
        executor.submit(check_new_invoices);
        executor.submit(check_new_receipts);
        executor.submit(print_invoices_thread);
                ...
                if (existing_invoice_data == null) {
                    ...
                    if (invoice_data_values != null) {
                        ...
                        if (file_name.equalsIgnoreCase(existing_duplicate_invoice_name)) {
                            //Re-print the earlier signed invoice
                            if (db_manager.reset_invoice_printed_status(file_name, 0)) {
                                System.out.println(TAG + ", Duplicate invoice (" + file_name + ") set for re-printing.");
                                status = "duplicate";
                            } else {
                                System.out.println(TAG + ", Failed to set duplicate invoice (" + file_name + ") for re-printing.");
                                status = null;
                            }
                        } else {
    ...
    class CheckAndPrintInvoicesThread implements Runnable {
        String TAG = "CheckAndPrintInvoicesThread";
        
        @Override
        public void run() {
            while (stop_background_tasks == false) {
                //Check for signed invoices ready for printing
                Future<HashMap<String, ArrayList<String>>> check_response = executor.submit(new CheckUnPrintedInvoicesTask());
                try {
                    //Block & get response
                    HashMap<String, ArrayList<String>> unprinted_invoices_found = check_response.get();
                    if (unprinted_invoices_found != null && unprinted_invoices_found.size() > 0) {
                        System.out.println(TAG + ", " + unprinted_invoices_found.size() + " , unprinted invoices found.");
                        HashMap<String, Future> printing_responses_map = print_invoices(unprinted_invoices_found);
                        if (printing_responses_map != null && printing_responses_map.size() > 0){
                            //Submit task to check for sending responses
                            executor.submit(new CheckPrintingResponsesTask(printing_responses_map));
                        }else{
                           System.out.println(TAG + ", printing_responses_map is null or empty..."); 
                        }
                    } else {
                        System.out.println(TAG + ", no unprinted invoices found.");
                        Thread.sleep(4000);
                    }
                } catch (InterruptedException ex) {
                    Logger.getLogger(BackgroundWorker.class.getName()).log(Level.SEVERE, null, ex);
        ...
        HashMap<String, Future> print_invoices(HashMap<String, ArrayList<String>> unprinted_invoices_found){
            //Submit invoices for printing
            int loop_count = 0;
            String invoice_name = null;
            Future<Boolean> printing_response = null;
            HashMap<String, Future> printing_responses_map = new HashMap();
            DbManager db_manager = new DbManager();
            for (Object key : unprinted_invoices_found.keySet()) {
                invoice_name = (String) key;
                System.out.println(TAG + ", printing : " + invoice_name + " loop_count: " + loop_count);
                //Submit the PrintInvoiceTask to executor
                printing_response = executor.submit(new PrintInvoiceTask(invoice_name));
                //Set printing progress in db
                if (db_manager.set_invoice_printing_status(invoice_name, "in progress")) {
                    //Add this future response to printing_responses_map
                    printing_responses_map.put(invoice_name, printing_response);
                } else {
                    //Add this future response to printing_responses_map, anyway
                    printing_responses_map.put(invoice_name, printing_response);
                }

            }//End for loop

            return printing_responses_map;
        }

        class CheckUnPrintedInvoicesTask implements Callable<HashMap<String, ArrayList<String>>> {

            @Override
            public HashMap<String, ArrayList<String>> call() throws Exception {
                DbManager db_manager = new DbManager();

                int initial_print_queuee_size = 0;
                if (print_queuee != null) {
                    initial_print_queuee_size = print_queuee.size();
                } else {
                    print_queuee = new HashMap();
                }

                print_queuee = db_manager.get_unprinted_invoices();
                if (print_queuee != null && print_queuee.size() > initial_print_queuee_size) {
                    return print_queuee;
                } else {
                    return null;
                }
        ...
        class CheckPrintingResponsesTask implements Callable<Integer> {
            HashMap<String, Future> printing_responses_map = null;
            
            public CheckPrintingResponsesTask(HashMap<String, Future> printing_responses_map){
                this.printing_responses_map = printing_responses_map;
            }
            ...
            public Integer call() throws Exception {
                ...
                if (printing_responses_map != null && printing_responses_map.size() > 0) {
                    DbManager db_manager = new DbManager();
                    for (Object key : printing_responses_map.keySet()) {
                        String invoice_name = (String) key;
                        //Get the current future
                        Future<Boolean> printing_response = printing_responses_map.get(key);
                        boolean response_received = false;
                        int total_trials = 0;
                        while (response_received == false) {
                            System.out.println(TAG + ", checking printing response for invoice: " + invoice_name + " , total_trial: " + total_trials);
                            //Increment total_trials
                            total_trials++;

                            //Block & check for response
                            try {
                                boolean sending_status = printing_response.get(3, TimeUnit.SECONDS);
                                //Update the sent_status & progress in db
                                int sent_status = 0;
                                if (sending_status) {
                                    sent_status = 1;
                                ...
                                //Update invoice sent flag in db
                                if (db_manager.set_invoice_print_status(invoice_name, sent_status)) {
                                    if (sent_status == 1) {
                                        //Set total_signed_invoices_sent
                                        total_signed_invoices_printed += 1;
                                        System.out.println(TAG + ", signed invoice: " + invoice_name + " printed successfully.");
                                        //Delete the sending failure log if it had been created
                                        app.Logger log = new app.Logger();
                                        log.deleteLog(invoice_name);
                                        //Remove from print_queuee
                                        print_queuee.remove(invoice_name);
                                    } else {
                                        System.out.println(TAG + ", signed invoice: " + invoice_name + " printing failed.");
                                        app.Logger log = new app.Logger();
                                        ArrayList<String> info_lines = new ArrayList();
                                        info_lines.add(TAG + ", signed invoice: " + invoice_name + " printing failed.");
                                        log.writeLog(invoice_name, info_lines);
                                        //Re-submit this file for re-sending by removing it from outbox
                                        System.out.println(TAG + ", current print_queuee with: " + invoice_name);
                                        System.out.println(print_queuee);
                                        print_queuee.remove(invoice_name);
                                        System.out.println(TAG + ", new print_queuee without: " + invoice_name);
                                        System.out.println(print_queuee);

                                    }
                                } else {
                                    System.out.println(TAG + ", failed to update printed status in db for: " + invoice_name);
                                }
                                //Set response_received flag
                                response_received = true;
                                //Set total_responses_received counter
                                ...
                                System.out.println(TAG + ", rechecking for response at total_trials: " + total_trials);
                                if (total_trials >= 30) {
                                    //Update or reset invoice printed flag in db
                                    if (db_manager.set_invoice_print_status(invoice_name, 0)) {
                                        System.out.println(TAG + ", signed invoice: " + invoice_name + " printing failed.");
                                        app.Logger log = new app.Logger();
                                        ArrayList<String> info_lines = new ArrayList();
                                        info_lines.add(TAG + ", signed invoice: " + invoice_name + " printing failed.");
                                        ...
                                        System.out.println(TAG + ", failed to reset printed status in db for: " + invoice_name);
                                    }
                                    //End this loop & move to the next email
                                    break;
                                }
                    ...
                    System.out.println(TAG + ", no printing responses to check.");
